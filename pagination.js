...
function pagination(total_results, start_number, results_page = 10) {
  ...
  const allNumbers = [total_results, results_page, start_number].every(
    Number.isFinite
  );
    ...
    if (page_num == "...") {
      ...
      if (page_num == cp) {
        ...
        if (cp > 5) {
          ...
          paginate.insertAdjacentHTML(
            "afterbegin",
            `<span class="page-link"><a href='${url.toString()}?${params.toString()}'>&#9756;</a></span>`
          );
        }
        ...
        if (cp < pages_list[pages_list.length - 1] - 4) {
          ...
          paginate.insertAdjacentHTML(
            "afterend",
            `<div class="page-link next-link"><a href='${url}?${params.toString()}'>&#9758;</a></div>`
          );
        }
        ...
        paginate.insertAdjacentHTML(
          "beforeend",
          `<span class="page-link current-page-link btn-floating btn-large">${page_num}</span>`
        );
        continue;
      }
      ...
      paginate.insertAdjacentHTML(
        "beforeend",
        `<span class="page-link"><a href='${url}?${params.toString()}'>${page_num}</a></span>`
      );
    }
  }
}
